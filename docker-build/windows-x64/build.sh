...
docker build $NO_CACHE -t bitcoinpurple-builder:windows-x64 -f "$SCRIPT_DIR/Dockerfile" "$SCRIPT_DIR"

# Create ccache and depends volumes if needed
if ! docker volume inspect bitcoinpurple_ccache_windows >/dev/null 2>&1; then
    log_info "Creating ccache volume..."
    docker volume create bitcoinpurple_ccache_windows
fi

if ! docker volume inspect bitcoinpurple_depends_windows >/dev/null 2>&1; then
    log_info "Creating depends volume..."
    docker volume create bitcoinpurple_depends_windows
fi
