#!/bin/bash
###############################################################################
# BitcoinPurple Core - Linux ARMv7 Docker Build Wrapper
###############################################################################
...
OUTPUT_DIR="$PROJECT_ROOT/build-output/linux-armv7"
...
docker build $NO_CACHE -t bitcoinpurple-builder:linux-armv7 -f "$SCRIPT_DIR/Dockerfile" "$SCRIPT_DIR"

# Create ccache and depends volumes if needed
if ! docker volume inspect bitcoinpurple_ccache_armv7 >/dev/null 2>&1; then
    log_info "Creating ccache volume..."
    docker volume create bitcoinpurple_ccache_armv7
fi

if ! docker volume inspect bitcoinpurple_depends_armv7 >/dev/null 2>&1; then
    log_info "Creating depends volume..."
    docker volume create bitcoinpurple_depends_armv7
fi
...
docker run --rm \
    ...
    -v "$OUTPUT_DIR:/output" \
    -v bitcoinpurple_ccache_armv7:/ccache \
    -v bitcoinpurple_depends_armv7:/workspace/depends \
